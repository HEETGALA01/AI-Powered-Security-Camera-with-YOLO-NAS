...
def main():
    ...
    app_mode = st.sidebar.selectbox(
        'Choose the App Mode', ['About App', 'Detection'])
    ...
    if app_mode == 'About App':
        ...
        DEMO_IMAGE = '..\train\images\image (5).jpg'

        if img_file_buffer is not None:
            img = cv2.imdecode(np.fromstring(
                img_file_buffer.read(), np.uint8), 1)
        ...
        ImageDetection(img, confidence, st)
        ...
        DEMO_VIDEO = 'AccidentData\accident.mp4'
        ...
        st.markdown("<hr/>", unsafe_allow_html=True)
        VideoDetection(
            tffile.name, kpi1_text, kpi2_text,  kpi3_text, stframe)
